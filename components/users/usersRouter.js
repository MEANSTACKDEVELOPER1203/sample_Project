...
router.get("/getCelebSearch/:userID/:string/:createdAt", function (req, res, next) {
  ...
  //const regex = new RegExp(escapeRegex(searchString), 'i');
  //searchString = searchString.replace(/\s\s+/g, ' ');
  searchString = req.params.string.toLowerCase();
  // console.log(searchString)
  let id = req.params.userID;
  let isCeleb = true;
      ...
      User.aggregate(
        ...
        function (err, data) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            currentCelebId = "" + currentCelebId;
            if (currentCelebId == id) {
              // console.log("I am in serach list hahahhaa")
              data.splice(i, 1);
              break;
            }
          }
          ...
          for (let j = 0; j < data.length; j++) {
            ...
            if (data[j].lrank == 0 && data[j].frank == 0)
              fArray.push(data[j])
            else if (data[j].lrank == 0)
              lArray.push(data[j]);
            else
              fArray.push(data[j])
          }
          // console.log("F SIZE==== ", fArray.length);
          // console.log("L SIZE==== ", lArray.length);
          if (fArray.length > 0)
            paginationDate = fArray[fArray.length - 1].created_at;
  ...
  User.findById(celebrityId, { password: 0 }, (err, userDetails) => {
    if (err) {
      ...
      MemberPreferences.aggregate([
        ...
        //new  celebrities: { $elemMatch: { CelebrityId: ObjectId(id), isFollower: true } }
        //old "celebrities.CelebrityId": celebrityId, "celebrities.isFan": true
        // old "celebrities.CelebrityId": celebrityId, "celebrities.isFollower": true
        {
          "$facet": {
            "Followers": [
              { "$match": { celebrities: { $elemMatch: { CelebrityId: celebrityId, isFollower: true } } } },
        ...
        slotMaster.find(query, (err, slotDetails) => {
          if (err) {
            ...
            if (err) {
              console.log(err)
              res.json({ success: 0, message: `Please enter valid email ${err}`, data: { userDetails: userDetails, fanFollowingFollowerFeedCount: null, celebContracts: null, memberMedia: null } })
            }
            else {
                  ...
                  celebrityContract.aggregate([
                    ...
                    if (err) {
                      ...
                      MemberMedia.aggregate([
                        ...
                        if (err) {
                          console.log(err)
                          res.json({ success: 0, message: `Please enter valid email ${err}`, data: { userDetails: userDetails, fanFollowingFollowerFeedCount: fanFollowingFollowerFeedCount, celebContracts: celebContracts, memberMedia: null } })
                        }
                        else {
                            ...
                            if (err) {
                              ...
                              MemberPreferences.aggregate(
                                [
                                  ...
                                  // console.log("AAAAAAAA", followerCount)
                                  if (err) {
                                    res.json({ success: 1, token: req.headers['x-access-token'], data: { userDetails: userDetails, fanFollowingFollowerFeedCount: fanFollowingFollowerFeedCount, celebContracts: celebContracts, memberMedia: memberMedia, creditDetails: creditDetails[0] } })
                                  } else {
                                    if (followerCount[0] && followerCount[0].followerCount) {
                                      fanFollowingFollowerFeedCount.Followers = followerCount[0].followerCount;
                                    }
                                    MemberPreferences.aggregate(
                                      [
                                        ...
                                        if (fancount[0] && fancount[0].fancount) {
                                          console.log("2");
                                          fanFollowingFollowerFeedCount.fanOfUr = fancount[0].fancount;
                                          res.json({ success: 1, token: req.headers['x-access-token'], data: { userDetails: userDetails, fanFollowingFollowerFeedCount: fanFollowingFollowerFeedCount, celebContracts: celebContracts, memberMedia: memberMedia, creditDetails: creditDetails[0], scheduleCount: parseInt(count) } })
                                        }
                                        else {
                                          console.log("3");
                                          res.json({ success: 1, token: req.headers['x-access-token'], data: { userDetails: userDetails, fanFollowingFollowerFeedCount: fanFollowingFollowerFeedCount, celebContracts: celebContracts, memberMedia: memberMedia, creditDetails: creditDetails[0], scheduleCount: parseInt(count) } })
...
//get deatils of own progfile
router.get("/getAllDetailsOfCelebrity/:celebrityId/:limit", userController.getAllDetailsOfCelebrity)
//get details of own profile end
...
router.post('/getCurrentToken', (req, res) => {
  if (req.body.userId) {
      ...
      if (err) {
        ...
        User.findOne({ _id: ObjectId(req.body.userId) }, { _id: 1 }, (err, userInfo) => {
          if (err) {
            ...
            res.json({ success: 1, data: { loginInfo: loginInfo, userInfo: userInfo } })
          }
        })
      }
    })
    ...
    res.json({ success: 0, messages: 'please provide email' })
  }
})
// End of Get Online Celebrities List
