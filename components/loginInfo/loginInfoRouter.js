                ...
                if (err) {
                  ...
                  Credits.findOne({
                    ...
                    if (err)
                      return res.json({ token: req.headers['x-access-token'], success: 0, message: err });
                    if (cBal) {
                      cBal.cumulativeCreditValue = cBal.cumulativeCreditValue + cBal.memberReferCreditValue
                      let creditInfo = cBal;
        ...
        if (err) {
          ...
          if (nResult.isCeleb == true) {
            ...
            liveTimeLog.create(liveTimeLogInfo, (err, liveTimeLogObj) => {
              if (err)
                console.log("****** Error while update celeb status ********", err)
              else {
                console.log("live ststus update succ....")
              }
            })
          }
        }
