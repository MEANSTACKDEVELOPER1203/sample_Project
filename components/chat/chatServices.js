...
const findChatsByCurrentMember1 = query =>
    new Promise((resolve, reject) => {
        Chat.aggregate([
            {
                $match: { chatRoomId: { $in: query.listOfChatRoomId } }
            },
            { $sort: { createdAt: -1 } },
            {
                $group:
            ...
            .then(listOfChatsObj => resolve(listOfChatsObj))
            .catch(err => reject(err))
    })

const findUnReadMessage = unReadMsgQuery =>
    new Promise((resolve, reject) => {
        // console.log(unReadMsgQuery);
        Chat.aggregate([
            {
                $match: {
                    chatRoomId: { $in: unReadMsgQuery.listOfChatRoomId }, receiverId: ObjectId(unReadMsgQuery.memberId), isRead: false
                }
            },
            {
                $group:
...
const findAllChatRoomForCurrentMember1 = memberId =>
    new Promise((resolve, reject) => {
        Chat.distinct("chatRoomId", { $or: [{ receiverId: ObjectId(memberId) }, { senderId: ObjectId(memberId) }], chatStatus: "Active" })
            .then(listOfChatRoomId => resolve(listOfChatRoomId))
            .catch(err => reject(err))
    })
...
var countNumberOfTodayMessage = function (query, callback) {
    ...
    Chat.countDocuments(query, (err, todayChatMessageRecords) => {
        if (!err)
...
let chatServices = {
    ...
    //async
    findAllChatRoomForCurrentMember1: findAllChatRoomForCurrentMember1,
    findChatsByCurrentMember1: findChatsByCurrentMember1,
