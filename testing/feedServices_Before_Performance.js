...
let findFeedByMemberIdNew = function (query, callback) {
    ...
    Feed.aggregate([
        ...
        if (err) {
            ...
            // function foo(cb) {
            //     let result = [];
            //     listOfFeedObj.forEach(e => {
            //         let editObj = {};
            //         editObj = e;

            //         // console.log("Feed member ID ", editObj.memberId);
            //         User.findOne({ _id: e.memberId }, function callback(err, userObj) {
            //             if (err) {
            //                 cb(err, null);
            //             } else {
            //                 console.log("From user",userObj._id);
            //                 let uObj = {};
            //                 uObj._id = userObj._id;
            //                 uObj.avtar_imgPath = userObj.avtar_imgPath;
            //                 uObj.country = userObj.country;
            //                 uObj.firstName = userObj.firstName;
            //                 uObj.gender = userObj.gender;
            //                 uObj.isCeleb = userObj.isCeleb;
            //                 uObj.lastName = userObj.lastName;
            //                 uObj.profession = userObj.profession;
            //                 uObj.username = userObj.username;
            //                 // uObj.cover_imgPath = userObj.cover_imgPath;
            //                 uObj.isManager = userObj.isManager;
            //                 editObj.feedByMemberDetails = uObj;
            //                 MediaTracking.count({ feedId: e._id, activities: "views" }, function callback(err, likeCount) {
            //                     if (err) {
            //                         cb(err, null);
            //                     } else {
            //                         console.log("Feed like Count", likeCount)
            //                         result.push(editObj)
            //                         console.log("r length", result.length);
            //                         console.log("F lenght", listOfFeedObj.length)
            //                         if (listOfFeedObj.length == result.length)
            //                             cb(null, result)
            //                         // MediaTracking.count({ feedId: e._id, activities: "comment" }, function callback(err, commentCount) {
            //                         //     if (err) {
            //                         //         cb(err, null);
            //                         //     } else {
            //                         //         result.push(editObj)
            //                         //         // console.log("Feed comment Count", commentCount)
